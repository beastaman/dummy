...
//		list.forEach(consumer);
		...
		LambdaExpression lambdaEmpression = new LambdaExpression();

		list.forEach(lambdaEmpression::fun1);
...
//		employeeTOs.sort((a, b) -> b.getEmpId() - a.getEmpId());

		System.out.println();
		List<EmployeeTO> employeeTOs3 = employeeTOs.stream().sorted((a, b) -> {
			return a.getAge() - b.getAge();
		}).collect(Collectors.toList());

		employeeTOs3.forEach(System.out::println);

//		Map<String , Integer> map = employeeTOs.stream().collect(toMap(EmployeeTO::getName , EmployeeTO::getAge));
