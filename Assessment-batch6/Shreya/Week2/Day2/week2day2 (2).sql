...
Table alias: (temporary name to table)
...
Selection and Projection : db.<docName>.find({selection/conditions to be used},{<projection columns to be displayed})

.pretty() -> arrange in object form

db.inventory.find({$OR: [{status : 'A'},{status : 'C'}]})
